...
.sorted(Comparator.comparing(Student::getLastName)  //sorts by comparing last names first
.thenComparing(Student::getFirstName))              //then by first names
...
.sorted(Comparator.comparing(Student::getGpa) 
    ...
    .reversed()) //makes the sorted list descending for both the highest GPA then Credits
.limit(n) //limits the list to n students
.collect(Collectors.toList());
...
.orElse(null);
    ...
    .flatMap(s -> s.getCourses().stream()) //makes new list to make a stream for just courses
